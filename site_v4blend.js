...
(function(){
  ...
  const open = () => {
    ...
    panel.appendChild(el('div', {
      ...
      background:'linear-gradient(90deg, rgba(201,168,76,.45), rgba(201,169,110,.25) 60%, transparent)'
    }));
    ...
    if(logoEl){
      ...
      if(img) Object.assign(img.style, { height:'34px', width:'auto', filter:'drop-shadow(0 0 6px rgba(201,168,76,.2))' });
    ...
    nav.querySelectorAll('a').forEach((a, i) => {
      ...
      const link = el('a', {
        ...
        background: isCurrent ? 'rgba(201,168,76,.08)' : 'transparent',
        borderLeft: isCurrent ? '2px solid rgba(201,168,76,.7)' : '2px solid transparent',
        letterSpacing:'.01em',
        boxSizing:'border-box',
        opacity:'0',
      ...
      if(isCurrent){
        link.appendChild(el('span', {
          ...
          boxShadow:'0 0 6px rgba(201,168,76,.55)'
        }));
      }
    ...
    liveRow.appendChild(el('span', {
      ...
      background:'rgb(201,168,76)', display:'inline-block',
      boxShadow:'0 0 3px rgba(201,168,76,.5), 0 0 10px rgba(201,168,76,.15)'
    }));
